...
public:
	int n; // êîë-âî òî÷åê
	Polynom *spline; //ìàññèâ ïîëèíîìîâ äëÿ ïîñòðîåíèÿ êðèâîé

	double *X, *Y; //ìàññèâû êîîðäèíàò òî÷åê
	...
	void CreateSpline(double *_x, double *_y) {
		...
		double *F = new double[n + 1];  //ïðàâàÿ ÷àñòü óðàâíåíèé äëÿ âû÷èñëåíèÿ Ñi  
	...
	// ñ÷èòàåì ïðàâûå ÷àñòè ñèñòåìû óðàâíåíèé äëÿ íàõîæäåíèÿ Ñi
		for (int i = 2; i <= n - 1; i++)
			F[i] = 3 * ((Y[i + 1] - Y[i]) / H[i + 1] - (Y[i] - Y[i - 1]) / H[i]);

	// ñîñòàâèì ìàòðèöó äëÿ îïèñàíèÿ ñèñòåìû óðàâíåíèé äëÿ íàõîæäåíèÿ Ñi
		for (int i = 0; i < n + 2; i++) {	
			B[i] = 0;
			C[i] = 0;
			D[i] = 0;
		...
		// çàíóëÿåì ïåðâûé è ïîñëåäíèé êîýôôèöèåíòû
		Cvec[2] = 0;
		Cvec[n + 1] = 0;
	
		for (int i = 2; i <= n - 1; i++) {              // matr[ñòðîêà][ñòîëáåö]
			matr[i][i] = H[i] * Cvec[i];
			matr[i][i + 1] = 2 * (H[i] + H[i + 1]) * Cvec[i + 1];
			matr[i][i + 2] = H[i + 1] * Cvec[i + 2];
		}

	// ðåøàåì ïîëó÷åííóþ ñèñòåìó ìåòîäîì Ãàóññà

		for (int i = 2; i < n - 1; i++) {
			// âû÷èñëèì êîýôôèöèåíò äëÿ ñòðîêè è äîìíîæèì
			double coef = matr[i + 1][i + 1] / matr[i][i + 1];
			multByConst(matr[i], coef, n + 2);
			F[i] = F[i] * coef;
			// âû÷òåì ñòðîêó äîìíîæåííóþ íà êîýôôèöèåíò
